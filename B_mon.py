...
def predict_currency(image):
    ...
    if total > 0 and not np.isclose(total, 1.0, atol=0.05):
...
st.markdown('<div class="main-title">🇧🇭 Bahraini Banknote Classification</div>',unsafe_allow_html=True)
...
if image_source is not None:
    ...
    st.markdown(
        ...
        <div class="result-box">
            ...
            <div class="result-confidence">Confidence: {confidence:.2%}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    for currency, probability in sorted_results:
        st.write(f"**{currency}:** {probability:.2%}")
